...
// Dear Austin: refresh the guarded editor-preview script after the compatibility patch.
add_filter('script_loader_src', static function ($src) {
    if (strpos($src, '/give/build/donationFormRegistrars.js') !== false) {
        return add_query_arg('da_editor_compat', '1', $src);
    }
    return $src;
}, 20);
